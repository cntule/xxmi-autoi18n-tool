...
const checkSkip = (item) => {
    ...
    const hasSkipStart = leadingComments.some(item => item.type === 'CommentBlock' && item.value.includes('skip-i18n-start'));
    const hasLeadingSkipEnd = leadingComments.some(item => item.type === 'CommentBlock' && item.value.includes('skip-i18n-end'));
    const hasSkipNextLine = leadingComments.some(item => item.type === 'CommentLine' && item.value.includes('skip-i18n-next-line'));

    const trailingComments = _.get(item, 'trailingComments', []);
    const hasSkipEnd = trailingComments.some(item => item.type === 'CommentBlock' && item.value.includes('skip-i18n-end'));
...
const makeVisitor = ({options, messages, file, ext, codeType}) => {
    ...
    return {
        enter(path) {
            const {node} = path;
            const {hasSkipStart, hasSkipEnd, hasSkipNextLine, hasLeadingSkipEnd} = checkSkip(node);
            ...
            if (astEnter && astEnter({path, node, file, filePath: file.filePath})) {
                path.skip();
            }
        },
        /**
        ...
        ObjectProperty(path) {
            ...
            // 处理对象属性
            if (astObjectProperty && key.type === 'Identifier' && value.type === 'StringLiteral') {
                if (astObjectProperty({
                    path,
                    node,
                    key,
                    value,
            ...
            if (!node._skipI18n && baseUtils.isChinese(node.value)) {
                switch (parent.type) {
                    ...
                    case 'TSEnumMember':
                        break;
                    default:
                        if (parent.type === 'CallExpression' && _.get(parent, 'callee.object.name') === 'console') {
                            // 不处理console
                        } else {
                            node.extra.raw = replaceStatement({value: node.value, options, messages, ext, codeType})
                        }
