...
module.exports = {
  ...
  /**
   * 判断路径p1是否以p2结尾
   * @param p1 src/components/HelloWorld.vue
   * @param p2 components/HelloWorld.vue
   * @return {boolean}
   */
  isPathEndsWith(p1, p2) {
    return this.normalizePath(p1).endsWith(this.normalizePath(p2));
  },
  /**
   * 是否是中文
   * @param value 内容
   ...
   * @return {boolean}
   */
  hasCallI18nObjetMethod({ code, options }) {
    return new RegExp(`${options.i18nObjectMethod}`, "gm").test(code);
  },
  /**
   * 去掉国际化实例
   * @param code
   ...
   * @return {*}
   */
  removeInstance({ code, options }) {
    if (!options.i18nObjectMethod || !options.i18nInstance) {
      return code;
    }

    // 没有使用 i18n.t 的方法，需要去掉引入的实例
    if (!this.hasCallI18nObjetMethod({ code, options })) {
      code = code.replace(
        new RegExp(this.stringRegEscape(options.i18nInstance), "gm"),
        ""
      );
      return code;
    }
  ...
  injectInstance({ code, ext, options }) {
    this.removeInstance({ code, options });
    // 如果存在需要注入的文件 进行注入
    if (
      this.hasCallI18nObjetMethod({ code, options }) &&
      options.i18nInstance
    ) {
      const matchInstance = code.match(
        new RegExp(this.stringRegEscape(options.i18nInstance), "gm")
      );
      // 如果已经存在实例直接返回
  ...
  handleNestedTags({ code, tagName }, cb) {
    ...
    code = code.replace(
      ...
      (
        ...
        if (startTag) {
          ...
          value = `<${starTagName + "%%_" + tagsNo[starTagName]}${
            startTagCon || ""
          }>`;
          if (tagsNo[starTagName] !== undefined) {
    ...
    code = code.replace(
      new RegExp(
        `<(${tagName}%%_0)((?:\\s+[^>]*)|)>[\\s\\S]*?<\\/\\1>|<(${tagName}%%_0)((?:\\s+[^>]*)|)\\/*>`,
        "gim"
      ),
      (match) => {
        // 编号完成清除标签的编号
